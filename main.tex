...
This paper extends the original argument with three observations. First, stochasticity has moved the bottleneck from construction to validation. Second, the ``language'' we program in (natural language prompts) is untyped and mutable, reintroducing the evils of shared state through context ordering. Third, the tooling culture celebrates artisanal prompt crafting rather than disciplined system design. We treat these as symptoms of the Token Pit.
...
We also extend the framing to cost. Every manual prompt tweak typically requires: (1) re-running sample transcripts, (2) recalibrating retrieval heuristics, (3) rewriting brittle regex-based parsers, and (4) subjectively judging outputs. This is engineering effort spent on accidental, not essential, work. A disciplined interface for intents would make most of this tunable or automated.
...
Probabilistic context also undermines modularity. A retrieval module can leak irrelevant tokens that bias a downstream planner; a logging string can perturb the distribution. This mirrors the original ``action at a distance'' critique of mutable state, now amplified by a continuous probability simplex. Without an interface boundary that constrains inputs and outputs, compositionality collapses.
...
Evaluation itself must be rethought. Statistical confidence intervals, stratified benchmarks, and property-based testing against invariants (e.g., schema validity, citation presence) should replace anecdotal spot checks. Offline corpora provide stable distributions; online canaries catch drift. The goal is not determinism, but bounded stochastic behavior with measurable guarantees.
...
A companion rule is to surface failures as first-class. If decoding fails schema validation, the system should emit an error type, not silently coerce. This preserves referential transparency at the interface: the same inputs lead either to a valid object or a typed failure, never to ambiguous partial strings.

Finally, DIP demands that evaluation be attached to signatures. Each intent should carry tests: invariants (always return JSON), metamorphic properties (adding irrelevant facts should not change answers), and task metrics (accuracy/F1/ROUGE where applicable). This closes the loop between interface and verification.

\section{Engineering for DIP: Testing and Operations}
DIP is an architectural stance; it also needs operational practice. We outline a minimal stack:
\begin{itemize}[leftmargin=*]
  \item \textbf{Typed interfaces}: Every LLM call expressed as a signature with input/output schemas and enumerated modes.
  \item \textbf{Prompt compiler}: An automatic synthesizer/optimizer (e.g., DSPy) that searches wording/ordering given frozen evaluation sets.
  \item \textbf{Constrained decoding}: JSON mode or grammar-based decoding to enforce structure at generation time, not post-hoc.
  \item \textbf{Evaluation harness}: Stratified test sets (happy-path, adversarial, long-context) with statistical thresholds; property-based fuzzing for invariants.
  \item \textbf{Versioned contexts}: Hashes of prompts, retrieval policies, and model checkpoints recorded per release to enable rollback and diffing.
  \item \textbf{Canaries and shadowing}: Low-risk traffic mirrors to detect drift before full rollout.
\end{itemize}
These practices mirror the configuration discipline advocated for FRP: explicit dependencies, limited effects, and reproducibility.
...
One could extend this with evaluation hooks:
\begin{verbatim}
from dspy.testing import assert_schema, assert_invariants

@program
def translate_with_guards(text, lang, tone="plain"):
    draft = translate_with_clarification(text, lang, tone)
    assert_schema(draft)
    assert_invariants(draft, [no_pii, includes_required_terms])
    return draft
\end{verbatim}
Here the tests are attached to the intent, not bolted on via ad hoc scripts. Failures are explicit and typed, avoiding silent regressions.
...
Two caveats temper the DIP proposal. First, compilation targets (LLM checkpoints) evolve; even with typed interfaces, behavior can drift, necessitating continual evaluation and possibly ensemble techniques. Second, many tasks require free-form generation (poetry, creative ideation) where strict schemas may suppress value. For these, DIP can still bound behavior through content filters and lightweight contracts, but guarantees are weaker. Finally, large-scale evaluation remains expensive; automated optimizers risk overfitting to narrow benchmarks. We expect hybrid human-in-the-loop evaluation to remain necessary.
...
\bibitem{nato1968}
NATO Science Committee. Software Engineering: Report on a Conference Sponsored by the NATO Science Committee, Garmisch, Germany, 1968. Available at \url{http://homepages.cs.ncl.ac.uk/brian.randell/NATO/nato1968.pdf} (accessed December 2025).

\bibitem{brooks1987}
F. P. Brooks. No Silver Bullet: Essence and Accidents of Software Engineering. IEEE Computer 20(4), 1987. DOI: 10.1109/MC.1987.1663532.

\bibitem{moseley2006}
B. Moseley and P. Marks. Out of the Tar Pit. Software Architecture References, 2006. Available at \url{https://curtclifton.net/papers/MoseleyMarks06a.pdf} (accessed December 2025).

\bibitem{openai2023}
OpenAI. GPT-4 Technical Report. arXiv:2303.08774, 2023. DOI: 10.48550/arXiv:2303.08774.

\bibitem{liu2023lost}
H. Liu, H. Liu, R. Schlangen. Lost in the Middle: How Language Models Use Long Contexts. arXiv:2307.03172, 2023. DOI: 10.48550/arXiv:2307.03172.

\bibitem{langosco2021goal}
L. Langosco, D. Krasheninnikov, J. Uesato, et al. Goal Misgeneralization: Why Correct Specifications Aren't Enough For Correct Goals. arXiv:2109.00647, 2021. DOI: 10.48550/arXiv:2109.00647.

\bibitem{liang2023holistic}
P. Liang, R. Bommasani, T. Lee, et al. Holistic Evaluation of Language Models. arXiv:2211.09110v3, 2023. DOI: 10.48550/arXiv:2211.09110.

\bibitem{yao2023react}
S. Yao, J. Zhao, D. Yu, et al. ReAct: Synergizing Reasoning and Acting in Language Models. arXiv:2210.03629, 2023. DOI: 10.48550/arXiv:2210.03629.

\bibitem{khattab2024dspy}
O. Khattab, L. Zheng, M. Saied, et al. DSPy: Compiling Declarative Language Model Calls into Programs. arXiv:2310.03714v2, 2024. DOI: 10.48550/arXiv:2310.03714.

\bibitem{instructor2023}
J. Etemadi, A. Bshara. Instructor: Structured Output for Large Language Models with Pydantic. arXiv:2312.03722, 2023. DOI: 10.48550/arXiv:2312.03722.

\bibitem{andreas2022language}
J. Andreas. Language Models as Hypothesis Engines. arXiv:2212.14052, 2022. DOI: 10.48550/arXiv:2212.14052.

\bibitem{sutton2019bitter}
R. Sutton. The Bitter Lesson. Incomplete Ideas Blog, 2019. Available at \url{http://www.incompleteideas.net/IncIdeas/BitterLesson.html} (accessed December 2025).

\bibitem{lewis2020rag}
P. Lewis, E. Perez, A. Piktus, et al. Retrieval-Augmented Generation for Knowledge-Intensive NLP. arXiv:2005.11401, 2020. DOI: 10.48550/arXiv:2005.11401.
